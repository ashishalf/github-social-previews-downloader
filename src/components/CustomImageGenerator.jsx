    ...
    <div className="w-full max-w-2xl mx-auto mt-8 animate-slide-up">
      <div className="bg-ghCard border border-ghBorder rounded-lg overflow-hidden">
        ...
        <button
          ...
          className="w-full px-6 py-4 flex items-center justify-between hover:bg-ghBg/50 transition-colors"
        >
          <h2 className="text-xl font-semibold text-ghText flex items-center gap-2.5">
        ...
        {showForm && (
          <div className="px-6 pb-6 border-t border-ghBorder pt-6 animate-fade-in">
            ...
            {repoData && !dataLoading && (
              <>
                {/* Data Preview Badges */}
                {/* <div className="mb-5 p-4 bg-ghBg rounded-lg border border-ghBorder">
                  <p className="text-ghMuted text-[10px] mb-3 font-bold uppercase tracking-widest">
                    📊 Live Data from GitHub API
                  </p>
                  <div className="flex flex-wrap gap-2">
                    <Badge icon="⭐" value={formatNumber(repoData.stars)} label="Stars" />
                    <Badge icon="🍴" value={formatNumber(repoData.forks)} label="Forks" />
                    <Badge icon="👥" value={formatNumber(repoData.contributorsCount)} label="Contributors" />
                    {repoData.language && <Badge icon="●" value={repoData.language} label="Language" dotColor={LANG_COLORS[repoData.language]} />}
                  </div>
                </div> */}
                ...
                <div className="mb-6 rounded-lg overflow-hidden border-2 border-ghBorder">
                  <div style={{ width: '640px', height: '320px', overflow: 'hidden' }}>
                    <div
                      ref={templateRef}
                      style={{
                        width: '1280px',
...
// BADGE COMPONENT
// ═══════════════════════════════════════════
function Badge({ icon, value, label, dotColor }) {
  return (
    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-ghCard border border-ghBorder rounded-full">
      {dotColor ? (
        <span className="w-3 h-3 rounded-full inline-block" style={{ backgroundColor: dotColor }} />
      ) : (
        <span className="text-xs">{icon}</span>
      )}
      <span className="text-ghText text-xs font-bold">{value}</span>
      <span className="text-ghMuted text-[10px]">{label}</span>
    </div>
  );
}